...
//   intake.move(127);
 ...
 while (pros::millis() - startMillis < 1300 + 100) {
...
chassis.pid_wait();
callLBReset();
set_drive(-29 - 1, 2000, 110);
chassis.pid_wait_until(-14 - 1);
chassis.pid_speed_max_set(70);
// set_drive(-10.5, 1500, 0, 45); // move slower
chassis.pid_wait_until(-26 - 1);
mogoClamp.toggle(); //clamp mogo
chassis.pid_wait();
intake.move_voltage(12000);
...
void map_field_routine() {
    // Display mapping mode on controller
    master.clear();
    master.set_text(0, 0, "MAPPING MODE");
    master.set_text(1, 0, "Collecting map data...");
    
    // Initialize position if needed
    // This assumes the robot starts at a known position near the corner of the field
    // Adjust these values based on your actual starting position
    chassis.odom_xyt_set(12.0, 12.0, 0.0);  // 12 inches from each wall, facing forward (0 degrees)
    
    // Run the mapping routine
    mapper.runMappingRoutine();
    
    // Display completion message
    master.set_text(1, 0, "Mapping complete!");
    master.rumble("--");
}
...
void check_mapping_mode() {
    // Check if Y + Up on controller is pressed to enter mapping mode
    if (master.get_digital(pros::E_CONTROLLER_DIGITAL_Y) && 
        master.get_digital(pros::E_CONTROLLER_DIGITAL_UP)) {
        
        // Countdown timer to allow user to release buttons
        master.clear();
        master.set_text(0, 0, "MAPPING MODE");
        
        for (int i = 3; i > 0; i--) {
            master.set_text(1, 0, "Starting in " + std::to_string(i) + "...");
            master.rumble(".");
            pros::delay(1000);
        }
        ...
        // Exit autonomous after mapping
        master.set_text(1, 0, "Press any button");
