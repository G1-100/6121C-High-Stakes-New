...
int sgn=isBlue?1:-1;
 ...
 chassis.pid_wait_until(-10);
 chassis.pid_speed_max_set(65);
 chassis.pid_wait_until(-23 + 6);
 mogoClamp.toggle(); // Clamp second mogo
 chassis.pid_wait();
 chassis.pid_turn_set((129 + 1) * sgn, 127); // turn to go back a bit
 chassis.pid_wait();
 // set_drive(-11 - 3);
...
int sgn=isBlue?1:-1;
chassis.odom_xyt_set(0, 0, (180 - 34.18 + 1) * sgn);
...
chassis.pid_wait_until(-27.5);
intake.move(127);


intake.move_voltage(12000);
chassis.pid_turn_set((-43 + 1.5) * sgn, 90); // Turn to center line 2 stacks, first 2 stack there
chassis.pid_wait();
intake.move_voltage(12000);
set_drive(20.5 + 1, 1500, 110); // intake ring
chassis.pid_wait();
...
// chassis.pid_wait();
intake.move(127);
chassis.pid_turn_set((7) * sgn, 90); // turn to final 2 stack in this quarter
chassis.pid_wait();
set_drive(19, 3000);
chassis.pid_wait();

chassis.pid_turn_set(-270 * sgn, 110); 
chassis.pid_wait();
set_drive(24 - 1.5, 2000, 50, 120); // move to wall
chassis.pid_wait();

intake.move(127);
chassis.pid_turn_set((180 + 0.5) * sgn, 110);
chassis.pid_wait();
...
intakeUnstuckActivated = false;
//intakeLift.toggle();
set_drive(67 - 4, 3000, 0, 100 + 10); // intake both rings of middle 2 stack
chassis.pid_wait_until(10);
intakeLift.toggle();
...
chassis.pid_wait();
set_drive(-24 - 0.5, 200, 0, 90); // drive to other mogo
intakeUnstuckActivated = true;
chassis.pid_wait_until(-10);
chassis.pid_speed_max_set(65);
chassis.pid_wait_until(-22.5);
...
LBState = PROPPED; // Prop LB for preload
//LBRotation.set_position(4400);
ladybrown2.set_zero_position(-46 + 15 + 5);
ChangeLBState(EXTENDED); // Extend LB for AWS
pros::delay(200 - 25);
intake.move(-127);
pros::delay(200 + 150);
...
while (pros::millis() - startMillis < 1600) {
...
//set_drive(10);
//chassis.pid_wait();
set_drive(-16.5 - 28.75); // move back
chassis.pid_wait();
intakeUnstuckActivated = true;
...
set_drive(43); // move to middle stack
chassis.pid_wait_until(15);
intakeUnstuckActivated = false;
...
LBState = PROPPED; // Prop LB for preload
//LBRotation.set_position(4400);
ladybrown2.set_zero_position(-46 + 20 + 5);
ChangeLBState(EXTENDED); // Extend LB for AWS
pros::delay(200);
intake.move(-127);
pros::delay(275 - 50);
...
chassis.pid_turn_set((-45 + 2) * sgn, 90); // Turn to center line 2 stacks, first 2 stack there
chassis.pid_wait();
intake.move_voltage(12000);
intake.move_voltage(12000);
...
// Code for getting other ring in center 2 stacks
chassis.pid_turn_set((-5 -2.5) * sgn, 127);  // turns to other 2 stack in middle 2 stacks
chassis.pid_wait();
set_drive(13 + 1, 2000, 50, 120); // drive to other 2 stack
chassis.pid_wait();
chassis.pid_turn_set(-15 * sgn, 120); // turn to other 2 stack
chassis.pid_wait();
set_drive(-16.25 - 1, 2000, 50, 120); // drive away from other 2 stack
chassis.pid_wait();
...
chassis.pid_wait();
pros::delay(300 - 150);
set_drive(20 + 3.5, 2000, 50, 120); // move to color sort ring
chassis.pid_wait();
pros::delay(500 - 300);
...
chassis.pid_wait();
chassis.pid_drive_set(-9, 127); // Move back from corner
chassis.pid_wait();
// set_drive(-53 + 1.5 + 10, 127); // Drive out of corner
// chassis.pid_wait_until(-52 + 1.5 + 10);
...
// startColorUntil(1);
set_drive(67 + 5, 3000, 0, 110); // intake both rings of middle 2 stack
//intakeLift.toggle();
